...
```{r}
# real data data
xs <- readRDS('southLondon1.RData')
xn <- readRDS('northLondon1.RData')
```
...
# *6 Wednesday
# *7 Thursday
wed <- c(as.vector(sapply(0:8,function(i){i*7*96+96*5+(1:96)})),
         as.vector(sapply(9:38,function(i){i*7*96+96*5+(1:96)-4})),
         as.vector(sapply(39:44,function(i){i*7*96+96*5+(1:96)+2})))
...
x1 <- as.vector(matrix(xn.new, ncol = num_periods)[25:96,])
plot(seq(0, 18*num_periods-0.25, by = 0.25), x1, type = 'l', col = 'red', lty = 1, ylim = c(0,150), main = 'Delayed Tweets for North/South Railway Companies')

legend('topright', c('north', 'south'), col = c('red','blue'), lwd = 2, lty = c(1,3))

x2 <- as.vector(matrix(xs.new, ncol = num_periods)[25:96,])
lines(seq(0, 18*num_periods-0.25, by = 0.25), x2, type = 'l', col = 'blue', lty = 3)
...
z_sample <- matrix(data = sample(1:2, 2*num_periods*72, replace = TRUE), nrow = 2)
...
mean(beta1_store[500:5000,2])/sd(beta1_store[500:5000,2])
mean(beta1_store[500:5000,3])/sd(beta1_store[500:5000,3])
mean(beta1_store[500:5000,4])/sd(beta1_store[500:5000,4])
```


```{r}
plot(beta2_store[,1],type = 'l', ylim = c(-5,10), col = 1)
lines(beta2_store[,2], col = 'red')
lines(beta2_store[,3], col = 'blue')
lines(beta2_store[,4], col = 'purple')

mean(beta2_store[500:5000,2])/sd(beta2_store[500:5000,2])
mean(beta2_store[500:5000,3])/sd(beta2_store[500:5000,3])
mean(beta2_store[500:5000,4])/sd(beta2_store[500:5000,4])
```
