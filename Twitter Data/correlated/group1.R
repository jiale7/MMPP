...
while(TRUE) {
  ...
  matched_indices <- grepl("SouthernRailUK|FGW|londonmidland|GNRailUK|
  eastcoastUK|TfL", df$text, ignore.case = TRUE)
  df1 <- df[matched_indices, ]
  
  # convert time to the hour past epoch [1970-01-01 (UTC)] 
  # i.e. we will bin the results per hour
  
  hours <- floor( unclass( as.POSIXct(df1$timestamp)) / 900 )
  
  # regex the tweets
  rv <- aggregate(df1$text, by = list(hour_past_epoch = hours), FUN = function(x) {
